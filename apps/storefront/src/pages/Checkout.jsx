          ...
          try {
            ...
            const verifyRes = await fetch(`${API_URL}/orders/verify-payment`, {
              ...
              body: JSON.stringify({
                ...
                total: orderData.calculatedTotal,
                customerName: customerInfo.name,
                address: `${customerInfo.address1}, ${customerInfo.address2 ? customerInfo.address2 + ', ' : ''}${customerInfo.city}, ${customerInfo.state}, ${customerInfo.pincode}`
              })
            });
                ...
                {cart.map((item, idx) => (
                  <div key={`${item.id}-${idx}`} className="mini-cart-item">
                    ...
                    <div className="mini-item-details">
                      <h4>{item.name}</h4>
                      <p>Size: {item.size} {item.color ? `| Color: ${item.color}` : ''} | Qty: {item.quantity}</p>
                    </div>
